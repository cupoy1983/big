...
class ItemUpdateRequest
{
	...
	/** 
	 * 商品的集分宝返点比例。如：5 表示返点比例0.5%. 注意：返点比例必须是>0的整数，而且最大是90,即为9%.B商家在发布非虚拟商品时，返点必须是 5的倍数，即0.5%的倍数。其它是1的倍数，即0.1%的倍数。无名良品商家发布商品时，复用该字段记录集分宝宝返点比例，返点必须是对应类目的返点步长的整数倍，默认是5，即0.5%。注意此时该字段值依旧必须是>0的整数，注意此时该字段值依旧必须是>0的整数，最高值不超过500，即50%
	 **/
...
商城卖家默认付款减库存
	...
	public function setEtcDeliveryWay($etcDeliveryWay)
	{
		$this->etcDeliveryWay = $etcDeliveryWay;
		$this->apiParas["etc_delivery_way"] = $etcDeliveryWay;
	}

	public function getEtcDeliveryWay()
	{
		return $this->etcDeliveryWay;
	}
