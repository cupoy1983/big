<?php
define('QQ_SCOPE',"get_user_info,add_topic,add_share");
...
function addShare($appid,$access_token,$openid,$data)
{
	...
	$title = $data['title'];
	if(empty($title))
		$title = cutStr($data['content'],68);
		
	$add_share .= '&title='.urlencode($title);
	$add_share .= '&summary='.urlencode(cutStr($data['content'],280,''));
	$add_share .='&url='.urlencode($data['url']);
	$add_share .='&site='.$_FANWE['setting']['site_name'];
	$add_share .='&fromurl='.$FANWE['site_url'];

	if(!empty($data['img_url']))
		$add_share .= '&images='.urlencode($data['img_url']);
	
    $info = file_get_contents($add_share);
