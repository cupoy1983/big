...
function bindCacheSetting()
{
	...
	if($settings['group_admin_fid'] > 0)
	{
		$group = FDB::fetchFirst('SELECT * FROM '.FDB::table('forum').' WHERE fid = '.(int)$settings['group_admin_fid']);
		if($group)
		{
			$settings['group_admin_fname'] = $group['name'];
			$settings['group_admin_furl'] = FU('group/detail',array('fid'=>$group['fid']));
		}
	}
	
	unset($settings['log_app']);
	...
	$shou_js = @file_get_contents(FANWE_ROOT.'shou/shou_source.js');
	$domain = explode('.',$_SERVER['HTTP_HOST']);
	array_shift($domain);
    $domain = implode('.',$domain);
	$shou_js = str_replace(array("#SITE_URL#","#SITE_DOMAIN#","#SITE_NAME#"),array($_FANWE['site_url'],$domain,$settings['site_name']),$shou_js);
	@file_put_contents(FANWE_ROOT.'shou/shou.js', $shou_js);
	unset($shou_js);
