...
function bindCacheGoodscate()
{
	...
	while($data = FDB::fetch($res))
	{
		...
		{
			$tag['url_tag'] =str_replace(array('+',' ','/'), array('S0U0M','E0P0T0Y','P0OL0E'),base64_encode($tag['tag_name']));
			$tags[] = $tag;
	...
	while($data = FDB::fetch($res))
	{
		if(isset($categorys['all'][$data['cate_id']]))
			$relateds[$data['type']][$data['f_cate_id']] = $data['cate_id'];
	}
