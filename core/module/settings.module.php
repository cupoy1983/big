...
class SettingsModule {
    ...
    public function personal() {
        global $_FANWE;
        //判断第三方用户登录 无绑定用户
        if (substr($_FANWE['user']['email'], 0, 3) == 'sys') {

            $_FANWE['user']['email'] = null;
            //unset ($_FANWE['user']['email']);
            if (empty($_COOKIE['Show_Bind_Change']) && empty($_COOKIE['Is_Show_Bind_Change']))
                fSetCookie("Show_Bind_Change", 1);
        }
    ...
    public function savePersonal() {
        ...
        $user = array(
            ...
            'weibo' => htmlspecialchars(trim($_FANWE['request']['weibo'])),
            'introduce' => htmlspecialchars(trim($_FANWE['request']['introduce'])),
        );
                ...
                if ($vservice->validation($validate, $data)) {
                    if (FS('User')->getUserNameExists($data['user_name'])) {
                        ...
                        if ($integrate_id > 0 && !FS("Integrate")->getCheckUserName($user_name, $integrate_id)) {
                            ...
                            FDB::query('UPDATE ' . FDB::table('user_match') . ' SET user_name = \'' . FS("Words")->segmentToUnicode($data['user_name']) . '\' WHERE uid = ' . $_FANWE['uid']);
        ...
        //修改邮箱
        if (!empty($email) && !empty($_FANWE['user']['email']) && substr($_FANWE['user']['email'], 0, 3) == 'sys') {
            $data = array(
                'email' => $email
            );
            ...
            if ($vservice->validation($validate, $data)) {
                if (FS('User')->getEmailExists($data['email'])) {
                    ...
                    if ($integrate_id > 0 && !FS("Integrate")->getCheckEmail($data['email'], $integrate_id)) {
                        $msg = FS("Integrate")->getInfo();
                    } else {
                        if ($integrate_id > 0) {
                            FS("Integrate")->editUser($integrate_id, '', $data['email'], '');
                        }
    ...
    public function password() {
        ...
        $keyid = FDB::resultFirst('SELECT keyid FROM ' . FDB::table('user_bind') . ' WHERE uid=' . $_FANWE['user']['uid']);
        ...
        if ($_FANWE['user']['password'] == md5('sys_' . substr(strtolower(md5($keyid)), 0, 6))) {
            include template('page/settings/settings_password_1');
        }else
     ...
     * @global type $_FANWE 
     */
    public function savePasswordBind() {
        global $_FANWE;
        $data = array(
            'newpassword' => $_FANWE['request']['newpassword'],
            'confirm_password' => $_FANWE['request']['newpasswordagain'],
