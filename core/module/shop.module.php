...
class ShopModule
{
	...
	public function show()
	{
		...
		$_FANWE['PAGE_SEO_SELF'] = $shop;
		unset($_FANWE['PAGE_SEO_SELF']['data']);
		if(isset($_FANWE['cache']['shops']['all'][$shop['cate_id']]))
		{
			$_FANWE['PAGE_SEO_SELF']['cate_name'] = $_FANWE['cache']['shops']['all'][$shop['cate_id']]['name'];
		}
		
		$cache_data = fStripslashes(unserialize($shop['data']));
        $shop['tags'] = $cache_data;
		if(empty($shop['taoke_url']))
			$shop['to_url'] = $shop['shop_url'];
		else
			$shop['to_url'] = $shop['taoke_url'];
		...
		if($share_count > 0)
		{
			$sql = 'SELECT s.share_id,s.uid,s.content,s.collect_count,s.comment_count,s.create_time,s.relay_count,s.cache_data 
