...
class GroupModule{
	...
	function index()
	{
		...
		if(getCacheIsUpdate($cache_file,SHARE_CACHE_TIME,1))
		{
			...
			$hot_events = FS('Activity')->getHotActivitys(10);
			$hot_votes = FS('Vote')->getHotVotes(10);
	...
	function detail()
	{
		...
		$_FANWE['PAGE_SEO_SELF'] = $group_detail;
		unset($_FANWE['PAGE_SEO_SELF']['cache_data']);
		$_FANWE['PAGE_SEO_SELF']['content'] = cutStr($group_detail['content'],80,'');
		
		$seo_user = FS('User')->getUserById($group_detail['uid']);
		$_FANWE['PAGE_SEO_SELF']['user_name'] = $seo_user['user_name'];
		
		if(isset($_FANWE['cache']['forum_category'][$group_detail['cid']]))
		{
			$_FANWE['PAGE_SEO_SELF']['cate_name'] = $_FANWE['cache']['forum_category'][$group_detail['cid']]['cate_name'];
		}
