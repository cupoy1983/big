...
class ShopService
{
	...
	/**  
	 * 获取店铺所在分享分类
	 * @param string $shop_cid 店铺分类
	 * @return int
	 */
	public function getCid($type,$shop_cid)
	{
		global $_FANWE;
		$key = 'shop_cate_related_'.$type;
		Cache::getInstance()->loadCache($key);
		$ids = ShopService::getParentIds($type,$shop_cid);
		array_unshift($ids,$shop_cid);
		foreach($ids as $id)
		{
	...
	public function getParentIds($type,$id)
	{
		...
		{
			$pids = FDB::resultFirst("SELECT pids FROM ".FDB::table('shop_cates')." WHERE type='$type' AND id='$id'");
			if(empty($pids))
				$pids = array();
			else
			{
