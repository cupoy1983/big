...
class AlbumService
{
	...
	public function deleteAlbumItem($share_id,$is_update = true)
	{
		...
		FS('Share')->deleteShare($share_id,true,$is_update);
	}
	...
	public function updateAlbum($aid)
	{
		...
		$share_count = (int)FDB::resultFirst('SELECT COUNT(share_id) FROM '.FDB::table('album_share_index').' WHERE album_id = '.$aid);
		...
		if(count($ids) > 0)
		{
			...
			while($data = FDB::fetch($res))
			{
				...
				foreach($data['cache_data']['imgs']['all'] as $img)
				{
	...
	public function albumImagesFormat(&$albums)
	{
		$image_ids = array();
		foreach($albums as $key => $album)
		{
			$albums[$key]['imgs'] = array();
			if(!empty($album['cache_data']))
			{
				$first_imgkey = NULL;
				$cache_data = fStripslashes(unserialize($album['cache_data']));
				$imgs = $cache_data['imgs'];
				foreach($imgs as $ikey => $img)
				{
					if($first_imgkey === NULL)
						$first_imgkey = $ikey;
					
					$albums[$key]['imgs'][$ikey] = $img;
					$image_ids[$img['img_id']][] = &$albums[$key]['imgs'][$ikey];
				}
				unset($albums[$key]['cache_data']);
			}
			
			$albums[$key]['img'] = array();
			if(!empty($album['best_img']))
				$albums[$key]['img']['img'] = $album['best_img'];
			else
			{
				if(count($albums[$key]['imgs']) > 0)
					$albums[$key]['img'] = &$albums[$key]['imgs'][$first_imgkey];
			}
		}
	...
	public function getIndexAlbums($num = 3)
	{
		...
		while($data = FDB::fetch($res))
		{
			...
			if(!empty($data['flash_img']))
				$album_list[$data['id']]['img']['img'] = $data['flash_img'];
			else
			{
				if(!empty($data['cache_data']))
				{
					$cache_data = fStripslashes(unserialize($data['cache_data']));
					$img = current($cache_data['imgs']);
					$album_list[$data['id']]['img'] = $img;
					$image_ids[$img['img_id']][] = &$album_list[$data['id']]['img'];
					unset($data['cache_data']);
				}
			}
		}
	...
	public function getBestAlbums($num = 6)
	{
		...
		AlbumService::albumImagesFormat($album_list);
		return $album_list;
	}
