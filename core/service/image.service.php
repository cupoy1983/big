...
class ImageService
{
	...
	public function addImage($image)
	{
		...
		if($server_img)
		{
			...
			unset($image['image_width'],$image['image_height']);
	...
	public function sendImageToServer($image,$id)
	{
		...
		if(parseUrl($image['src']))
		{
			...
			{
				$server_img = array();
				$server_img['width'] = $image['image_width'];
				$server_img['height'] = $image['image_height'];
				$server_img['server_code'] = '';
			}
			else
			{
				$server_img = copyImage($image['src'],false,'temp');
				@unlink($server_img['path']);
			}
			$server_img['path'] = $image['src'];
			$server_img['url'] = $image['src'];
		}
		elseif(!empty($image['server_code']))
		{
			$server_args['id'] = $id;
