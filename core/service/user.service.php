...
class UserService
{
	...
	public function setSession($user,$life = 0)
	{
		...
		if(MODULE_NAME == 'callback')
		{
			...
			$sql = "SELECT {$user_field} FROM ".FDB::table('user')." WHERE uid = ".$user['uid'];		
			$integrate_id = (int)FDB::resultFirst($sql);
	...
	public function usersFormat(&$uid_list)
	{
		...
		if(count($uids) > 0)
		{
			...
			if(count($diffs) > 0)
			{
				...
				if(!empty($diffs))
				{
					$res = FDB::query("SELECT u.uid,u.user_name,u.avatar,u.reg_time,uc.* FROM ".FDB::table('user').' AS u 
						INNER JOIN '.FDB::table('user_count').' uc USING(uid) 
						WHERE u.uid IN ('.$diffs.')');
					while($item = FDB::fetch($res))
					{
		...
		if(abs($score) > 0)
		{
			...
			if($is_update !== false)
			{
				if($is_log)
				{
					...
					$log['content'] = addslashes($log['content']);
					$log['rec_id'] = $rec_id;
	...
	public function getZoneFollowUsers($num = 8)
	{
		global $_FANWE;
		$num = (int)$num;
		if($_FANWE['setting']['is_show_follow'] == 0 || !$num)
			return array();
		
		$list = array();
		if(	$_FANWE['setting']['is_show_follow'] == 1)
		{
			$sql = 'SELECT u.uid,u.user_name,u.avatar,ud.reason,up.introduce,uc.fans,uc.shares,uc.goods 
				...
				WHERE ud.status=1 and ud.uid <> '.$_FANWE['uid'].' 
				ORDER BY ud.id DESC LIMIT 0,'.$num;
			
			$res = FDB::query($sql);
			while($data = FDB::fetch($res))
			{
				if(!empty($data['reason']))
					$data['introduce'] = $data['reason'];
				unset($data['reason']);
				$list[] = $data;
			}
		}
		...
		if($_FANWE['setting']['is_show_follow'] == 2)
		{
			$sql = 'SELECT u.uid,u.user_name,u.avatar,up.introduce,uc.fans,uc.shares,uc.goods 
				FROM '.FDB::table('user').' AS u 
				INNER JOIN '.FDB::table('user_count').' AS uc ON uc.uid = u.uid 
				INNER JOIN '.FDB::table('user_profile').' AS up ON up.uid = u.uid 
				WHERE u.status=1 and u.uid <> '.$_FANWE['uid'].' AND avatar > 0 
				ORDER BY u.uid DESC LIMIT 0,'.$num;
	 ...
	 * @return type 
	 */
	function sendUserActiveMail($user_id)
	{
		global $_FANWE;
		
		if($_FANWE['setting']['is_mail_activate']==1  || $_FANWE['setting']['bind_is_mail_activate']==1 )
		{
			$userinfo = FDB::fetchFirst("select * from ".FDB::table('user')." where uid=".$user_id);
			if($userinfo)
			{
				$activesn = strtoupper("U".md5(uniqid()));
				$userinfo['active_sn'] = $activesn;
				FDB::query("update ".FDB::table('user')." set active_sn = '".$activesn."',status=0 where uid=".$user_id);
				$userinfo['active_url'] = $_FANWE['site_url']."user.php?action=verify&sn=".$activesn;

				$args = array(
					'userinfo'=>&$userinfo,
					'site_name'=>$_FANWE['setting']['site_name']
				);
				$mail_html = tplFetch("inc/user/mail_template",$args);
				
				require_once fimport('mail.class','class');
				$mail = new Mail();
				$mail_info['type'] = 1; // 设置邮件格式为 HTML
				$mail_info['site_name'] = $_FANWE['setting']['site_name'];
				$mail_info['subject'] = '感谢注册'.$_FANWE['setting']['site_name'].'，请验证您的Email'; // 标题					
				$mail_info['body'] =  $mail_html; // 内容
				$mail_info['email'] = $userinfo['email'];
				...
				{
					//更新邮件发送次数
					FDB::query('update '.FDB::table('smtp').' set batch_count = batch_count+1 where id='.$stmp_arr['id']);
					return true;
				}
