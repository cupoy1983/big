...
class ShareService
{
	...
	/**
	 * 通过的分享提交表单的数据处理
	 * @param mix $post 为标准分享表单 $post['type'] default:默认,bar:主题,ershou:二手,ask:问答
	 * $post['share_data'] = photo 有图 goods 有产品 goods_photo:有图有商品 default:都没有
	 * 	* 返回
	 *  array(
	 *   'status' => xxx  状态  bool
	 *   'share_id' => share_id
	 ...
	 * )
	 */
	public function submit(&$post,$is_check = true,$is_score = true)
	{
		//创建分享数据
		global $_FANWE;
		$data = array();
		...
		$share_data['base_id'] = intval($post['base_id']);
		$share_data['albumid'] = intval($post['albumid']);
		...
		$share_data['title'] = isset($post['title']) ? htmlspecialchars(trim($post['title'])) : '';
		...
		if(isset($post['goods']) && is_array($post['goods']) && count($post['goods']) > 0)
		{
			$share_goods = $post['goods'];
			foreach($share_goods as $goods)
			{
				...
				if($goods['item']['id'] == 0)
				{
					...
					$c_data['shop_name'] = htmlspecialchars(strip_tags($goods['shop']['name']));
					...
					$c_data['shop_width'] = (int)$goods['shop']['image_width'];
					$c_data['shop_height'] = (int)$goods['shop']['image_height'];
					$c_data['shop_cid'] = (int)$goods['shop']['cid'];
				}
				else
				{
				...
				$c_data['price'] = (float)$goods['item']['price'];
				$c_data['commission'] = (float)$goods['item']['commission'];
				$c_data['content'] = $goods['item']['desc'];
				$c_data['type'] = $goods['item']['type'];
				$c_data['url'] = $goods['item']['url'];
				$c_data['goods_key'] = $gkey;
				$c_data['delist_time'] = $goods['item']['delist_time'];
				$c_data['name'] = htmlspecialchars(strip_tags($goods['item']['name']));
				$c_data['cid'] = (int)$goods['item']['cid'];
				$c_data['sort'] = isset($post['goods_sort'][$gkey]) ? intval($post['goods_sort'][$gkey]) : 10;
				$c_data['color'] = (int)$goods['item']['color'];
				$c_data['alt'] = isset($goods['item']['goodsalt']) ? $goods['item']['goodsalt'] : "";
				$c_data['goods_width'] = (int)$goods['item']['image_width'];
				$c_data['goods_height'] = (int)$goods['item']['image_height'];
		...
		if(empty($share_type) || !in_array($share_type,array('default', 'dapei', 'look')))
			$share_type = 'default';
		
		//创建图库数据
		$share_photos_data = array();
		if(isset($post['pics']) && is_array($post['pics']) && count($post['pics']) > 0)
		{
			$share_photos = $post['pics'];
			foreach($share_photos as $pkey => $photo)
			{
				$photo = fAddslashes(json_decode(base64_decode($photo),true));
				$c_data = array();
				$c_data['img'] = $photo['path'];
				$c_data['server_code'] = $photo['server_code'];
				$c_data['type'] = $share_type;
				$c_data['alt'] = isset($photo['photoalt']) ? $photo['photoalt'] : "";
				$c_data['sort'] = isset($post['pics_sort'][$pkey]) ? intval($post['pics_sort'][$pkey]) : 10;
				$c_data['image_width'] = (int)$photo['image_width'];
				$c_data['image_height'] = (int)$photo['image_height'];
		...
		if(isset($post['tags']) && trim($post['tags']) != '')
		{
		...
		$data['pub_out_check'] = intval($post['pub_out_check']);  //发送到外部微博
		...
		if(intval($share_id)>0)
		{
			...
			foreach($share_photo as $k=>$photo)
			{
				...
				if(!isset($photo['img_id']))
				{
					...
					$o_img['image_width'] = $photo['image_width'];
					$o_img['image_height'] = $photo['image_height'];
				...
				if($photo['img_id'] > 0)
				{
					...
					$share_photo_data['alt'] = str_replace(array('#','@'), array('＃','＠'), $photo['alt']);
			...
			if(isset($data['share_goods']))
			{
				...
				foreach($share_goods as $goods)
				{
					...
					if($goods['goods_id'] == 0)
					{
						if($shop_id == 0 && !empty($goods['shop_url']))
						{
							...
							if(intval($shop_id) == 0)
							{
								...
								if(!empty($goods['shop_logo']))
								{
									...
									$o_img['image_width'] = $goods['shop_width'];
									$o_img['image_height'] = $goods['shop_height'];
									$o_img = FS("Image")->addImage($o_img);
								...
								$shop_data['cate_id'] = $goods['shop_cid'];
						...
						$o_img['image_width'] = $goods['goods_width'];
						$o_img['image_height'] = $goods['goods_height'];
						$o_img = FS("Image")->addImage($o_img);
						...
						if(!empty($o_img))
						{
							...
							deleteTempCache(md5($goods['goods_key']));
					...
					if($goods['goods_id'] > 0)
					{
						...
						$share_goods_data['alt'] = str_replace(array('#','@'), array('＃','＠'), $goods['alt']);
						$share_goods_data['color'] = $goods['color'];
			...
			if(in_array($share_data_type,array('goods','photo','goods_photo')))
			{
				...
				if(!$is_rel_share)
				{
					...
					$share_colors = array_unique($share_colors);
					foreach($share_colors as $cid)
					{
						$cate_data = array();
						$cate_data['share_id'] = $share_id;
					...
					if(count($cate_tags) > 0)
					{
						...
						{
							if($tag_count >= $_FANWE['setting']['share_tag_count'])
								break;
			...
			if($data['pub_out_check'] && !defined('IS_COLLECT_GOODS'))
			{
				...
				if($is_open)
				{
					CronService::createRequest(array('m'=>'share','a'=>'sync_send'));
				}
			}
		}
		else
	...
	public function getCateTags($str,$cids)
	{
		...
		/*$cate_tags = array_slice($cate_tags,0);
	...
	public function deleteShare($share_id,$is_score = true,$update_user_cache = true)
	{
		...
		if(!empty($share))
		{
			...
			if($goods_count > 0 || $photo_count > 0)
			{
				...
				FDB::delete('share_color','share_id = '.$share_id);
			}
	...
	public function getShareDetail($share_id,$is_collect = false,$is_tag = false,$collect_count = 20)
	{
		...
		if($share)
		{
			...
			ShareService::shareImageFormat($share,0,false);
		}
		return $share;
	}
	...
	public function getShareDetailList($list,$is_parent = false,$is_collect = false,$is_tag = false,$is_comment = false,$comment_count = 10,$collect_count = 20,$is_user = false)
	{
		...
		if(count($rec_ids) > 0)
		{
			...
			foreach($intersects as $share_id)
			{
				...
				$format_images[] = &$rec_shares_ids[$share_id];
				$temp_ids[] = $share_id;
			}
		...
		if(count($format_images) > 0)
			ShareService::shareImagesFormat($format_images);
	...
	public function getShareImage($share_id,$data_type)
	{
		...
		{
			case 'goods':
				$sql = 'SELECT share_id,uid,id,goods_id,img_id,base_id,base_share,alt  
				...
				while($data = FDB::fetch($res))
				{
					if($data['goods_id'] > 0)
					{
						...
						$list['all'][$pkey]['img_id'] = $data['img_id'];
						$list['all'][$pkey]['goods_id'] = $data['goods_id'];
						$list['all'][$pkey]['alt'] = $data['alt'];
			...
			break;
			case 'photo':
				$sql = 'SELECT share_id,uid,id,img_id,type,base_id,base_share,alt 
			...
			break;
			case 'goods_photo':
				$sql = '(SELECT share_id,id,uid,img_id,goods_id,\'goods\' AS type,sort,base_id,base_share,alt FROM '.FDB::table('share_goods').'
					WHERE share_id = '.$share_id.')
					UNION
					(SELECT share_id,id,uid,img_id,0 AS goods_id,type,sort,base_id,base_share,alt FROM '.FDB::table('share_photo').'
					WHERE share_id = '.$share_id.')
					ORDER BY sort ASC';
				$res = FDB::query($sql);
				while($data = FDB::fetch($res))
				...
				case 'goods':
					...
					while($data = FDB::fetch($res))
					{
						if($data['goods_id'] > 0)
						{
							...
							$list[$data['share_id']]['all'][$pkey]['img_id'] = $data['img_id'];
							$list[$data['share_id']]['all'][$pkey]['alt'] = $data['alt'];
							$list[$data['share_id']]['all'][$pkey]['goods_id'] = $data['goods_id'];
	...
	public function shareImageFormat(&$share,$pic_num = 0,$uncache = true)
	{
		$image_ids = array();
		$goods_ids = array();
		$image_list = array();
		...
		if(isset($images['all']))
		{
			$is_more = count($images['all']) > 1;
			foreach($images['all'] as $ik => $img)
			{
				if($is_more)
					$img['url'] = FU('note/'.$img['type'],array('sid'=>$img['share_id'],'id'=>$img['id']));
				else
					$img['url'] = FU('note/index',array('sid'=>$img['share_id']));
				
				if($img['type'] == 'g')
					$img['is_image_goods'] = 1;
                                
                                if(!empty($img['alt']) && (strpos($img['alt'],'#') !== FALSE || strpos($img['alt'],'@') !== FALSE))
                                    $img['alt'] = str_replace(array('#','@'), array('＃','＠'), $img['alt']);
					
				$image_list[$image_index] = $img;
				$images['all'][$ik] = &$image_list[$image_index];
				$share['imgs'][] = &$image_list[$image_index];
				if($img['type'] != 'g')
					$share['photo_imgs'][] = &$image_list[$image_index];
				else
					$goods_ids[$img['goods_id']][] = &$image_list[$image_index];
				$image_ids[$img['img_id']][] = &$image_list[$image_index];
				$image_index++;
		...
		if($uncache)
			...
			unset($share['cache_data']['imgs']);
		FS('Image')->formatByIdKeys($image_ids);
		FS('Goods')->formatByIDKeys($goods_ids,false);
	}
	
	public function shareImagesFormat(&$shares,$pic_num = 0)
	{
		$image_ids = array();
		$goods_ids = array();
		$image_list = array();
		...
		foreach($shares as $key => $share)
		{
			...
			if(isset($images['all']))
			{
				...
				foreach($images['all'] as $ik => $img)
				{
					...
					$image_list[$image_index] = $img;
					$images['all'][$ik] = &$image_list[$image_index];
					$shares[$key]['imgs'][] = &$image_list[$image_index];
					if($img['type'] != 'g')
						$shares[$key]['photo_imgs'][] = &$image_list[$image_index];
					else
						$goods_ids[$img['goods_id']][] = &$image_list[$image_index];
					$image_ids[$img['img_id']][] = &$image_list[$image_index];
					$image_index++;
			...
			if(isset($images['goods']))
			{
				foreach($images['goods'] as $ik)
				{
					$shares[$key]['goods_imgs'][] = &$images['all'][$ik];
				}
			}
			...
			if(isset($images['dapei']))
			{
				foreach($images['dapei'] as $ik)
				{
					$shares[$key]['dapei_imgs'][] = &$images['all'][$ik];
				}
			}
			...
			if(isset($images['look']))
			{
				foreach($images['look'] as $ik)
				{
					$shares[$key]['look_imgs'][] = &$images['all'][$ik];
				}
			}
			...
			if(isset($images['default']))
			{
				foreach($images['default'] as $ik)
				{
					$shares[$key]['default_imgs'][] = &$images['all'][$ik];
				}
			}
			unset($shares[$key]['cache_data']);
		}
	...
	public function getPrevNextShares($uid,$share_id)
	{
		...
		$arr['prev'] = (int)FDB::resultFirst('SELECT share_id FROM '.FDB::table('share_images_index').' 
			WHERE uid = '.$uid.' AND share_id < '.$share_id.' ORDER BY share_id DESC LIMIT 1');
		
		$arr['next'] = (int)FDB::resultFirst('SELECT share_id FROM '.FDB::table('share_images_index').' 
			WHERE uid = '.$uid.' AND share_id > '.$share_id.' ORDER BY share_id ASC LIMIT 1');
		return $arr;
	}
    ...
    public function updateShareMatch($share_id)
    {
		...
		{
			$sql = 'SELECT g.*,s.shop_name FROM '.FDB::table('goods').' AS g 
				LEFT JOIN '.FDB::table('shop').' AS s ON s.shop_id = g.shop_id 
				WHERE g.id IN ('.implode(',',$goods_ids).')';
			$gres = FDB::query($sql);
			while($gdata = FDB::fetch($gres))
			{
				$content_match.=' '.$gdata['name'];
				if(!empty($gdata['shop_name']))
					$content_match.=' '.$gdata['shop_name'];
			}
		}
	...
	public function updateShareStatus($share_id,$status)
	{
		...
		if(in_array($share_data,array('goods','photo','goods_photo')))
		{
			...
			foreach($cache_data['imgs']['all'] as $image)
			{
				...
				{
					$is_rel_share = false;
					if((int)$image['goods_id'] > 0)
						$goods_ids[] = $image['goods_id'];
				}
			}
		}
		...
		if(count($goods_ids) > 0)
		{
			...
			while($gdata = FDB::fetch($gres))
			{
				if(isset($gdata['price']) && (float)$gdata['price'] > 0)
					$goods_prices[] = (float)$gdata['price'];
					
				if(isset($gdata['shop_id']) && (float)$gdata['shop_id'] > 0)
					$shop_ids[] = $gdata['shop_id'];
			}
		}
			...
			if(in_array($share_data,array('goods','photo','goods_photo')))
			{
				...
				if($type == 'album_item')
				{
					...
					if($album)
					{
						...
						FDB::insert("album_share_index",$album_share);
                                                FS('Album')->updateAlbum($album['album_id']);
					}
				}
			}
	...
	public function updateShareColor($share_id,$colors = array())
	{
		global $_FANWE;
		$share_id = (int)$share_id;
		if($share_id == 0)
		...
		FDB::query("delete from ".FDB::table("share_color")." where share_id = ".$share_id);
	...
	public function tagsFormat(&$tags)
	{
		if(isset($tags))
		{
			...
			{
                                $tag_name =str_replace(array('+',' ','/'), array('S0U0M','E0P0T0Y','P0OL0E'),base64_encode($tag['tag_name']));
				$tags[$tk]['url'] = FU('book/shopping',array('tag'=>$tag_name));
			}
		}
	}
	 ...
	 */
	public function saveRelay($post)
	{
		global $_FANWE;
	...
	if($tid > 0 && $is_no_post == 0)
		...
		$is_comment_base = isset($post['is_comment_base']) ? intval($post['is_comment_base']) : 0;
	...
	public function saveComment($post, $share)
	{
		...
		$is_relay = isset($post['is_relay']) ? intval($post['is_relay']) : 0;
		//转发分享
		if($is_relay == 1)
		{
			if($share['base_id'] > 0)
			{
				$share_user = FS('User')->getUserCache($share['uid']);
				$post['content'] = trim($post['content']).'//@'.$share_user['user_name'].':'.$share['content'];
			}
			//添加评论消息提示
	...
	public function getCategorysByShareId($share_id,$return_name = false)
	{
		$share_id = (int)$share_id;
		if($share_id == 0)
			return array();
		...
		global $_FANWE;
		$list = array();
		$res = FDB::query('SELECT * FROM '.FDB::table('share_category').' WHERE share_id = '.$share_id);
		while($data = FDB::fetch($res))
		{
			if(isset($_FANWE['cache']['goods_category']['all'][$data['cate_id']]))
			{
				$cate = $_FANWE['cache']['goods_category']['all'][$data['cate_id']];
				if($return_name)
					$list[] = $cate['cate_name'];
				else
					$list[] = $cate;
	...
	public function getShareCateOptions($pid = 0,&$html,$leval = 0)
	{
		global $_FANWE;
		$pad = $leval > 0 ? str_pad('&nbsp;',18 * $leval,'&nbsp;').'|--&nbsp;' : '';
		if($pid == 0)
		{
			foreach($_FANWE['cache']['goods_category']['parent'] as $cid)
			{
				$cate = $_FANWE['cache']['goods_category']['all'][$cid];
				$html .= '<option value="'.$cate['cate_id'].'" cateName="'.$cate['cate_name'].'">'.$pad.$cate['cate_name'].'</option>';
				if(isset($cate['child']))
				{
					foreach($cate['child'] as $cid)
					{
						ShareService::getShareCateOptions($cid,$html,$leval + 1);
					}
				}
			}
		}
		...
		{
			$cate = $_FANWE['cache']['goods_category']['all'][$pid];
			$html .= '<option value="'.$cate['cate_id'].'" cateName="'.$cate['cate_name'].'">'.$pad.$cate['cate_name'].'</option>';
			if(isset($cate['child']))
			{
				foreach($cate['child'] as $cid)
	...
	public function getRelatedSharesByShareIdAndTags($share_id,$tags)
	{
		global $_FANWE;
		$share_id = (int)$share_id;
		if($share_id == 0 || !is_array($tags) || count($tags) == 0)
			return array();
		
		$related_key = 'share/'.getDirsById($share_id).'/related';
		$list = getCache($related_key);
		if($list === NULL)
		{
			$cids = array();
			$res = FDB::query('SELECT * FROM '.FDB::table('share_category').' WHERE share_id = '.$share_id);
			...
			{
				$cate = $_FANWE['cache']['goods_category']['all'][$cid];
				$result = array_intersect($temp,$cate['childs']);
				if(!$result)
					$args['cids'][] = $cid;
			...
			foreach($tags as $tag)
			{
				foreach($cids as $cid)
				{
					...
					if($is_create)
						$args[] = array('cid'=>$cid,'tag'=>$tag);
					
					if(!empty($result['data']))
						$share_ids = array_merge($share_ids,$result['data']);
				}
			}
			...
			if(count($share_ids) > 0)
			{
				...
				if(count($share_ids) > 0)
				{
					$share_ids = array_slice($share_ids,0,32);
					$list = FDB::fetchAll('SELECT share_id,uid,content,collect_count,comment_count,create_time,cache_data FROM '.FDB::table('share').' WHERE share_id IN ('.implode(',',$share_ids).')');
					$list = ShareService::getShareDetailList($list);
			...
			if(count($args) > 0)
			{
				FS('Delay')->create(array('m'=>'share','a'=>'related','args'=>$args));
			}
		}
		return $list;
	}
