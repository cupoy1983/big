...
class Template {
	...
	function parseTemplate($tpl_file, $tpl_dir, $file, $cache_file)
	{
		...
		$template = preg_replace("/[\n\r\t]*\{getimgbyid\s+(\S+?)(?:\s+(\S+?)\s+(\S+?)(?:\s+(\S+?)(?:\s+(\S+?))?)?)?\}[\n\r\t]*/ies", "\$this->getImgByIdTags('\\1','\\2','\\3','\\4','\\5')", $template);
		...
		$template = preg_replace("/[\n\r\t]*\{avatar\s+(\S+?)(?:\s+(\S+?))?\}[\n\r\t]*/ies", "\$this->avatarTags('\\1','\\2')", $template);
		$template = preg_replace("/[\n\r\t]*\{cutstr\s+(.+?)\s+(.+?)(?:|\s(.+?))\}[\n\r\t]*/ies", "\$this->cutstrTags('\\1','\\2','\\3')", $template);
		$template = preg_replace("/[\n\r\t]*\{anchor\s+(.+?)(?:\s+(\d+?))?\}[\n\r\t]*/ies", "\$this->anchorTags('\\1','\\2')", $template);
	...
	function avatarTags($img_id,$type = 's')
	{
		if(empty($code))
			$code = '"'.$code.'"';
		...
		$this->replace_code['replace'][$i] = "<?php echo setTplAvatarFormat($img_id,'$type');?>";
		return $search;
	}
	...
	function anchorTags($str,$count)
	{
		$count = (int)$count;
		$i = count($this->replace_code['search']);
		$this->replace_code['search'][$i] = $search = "<!--ANCHOR_TAG_$i-->";
		$this->replace_code['replace'][$i] = "<?php echo formatAnchorLink($str,$count);?>";
		return $search;
	}
	 ...
	 * 解析模板中的getimgbyid标签
	 */ 
	function getImgByIdTags($id,$width=0,$height=0,$gen=0,$is_full=0)
	{
		if(!preg_match('/\$.+/i',$id))
			$id = (int)$id;
		...
		if(!preg_match('/\$.+/i',$is_full))
			$is_full = (int)$is_full;
		
		$i = count($this->replace_code['search']);
		$this->replace_code['search'][$i] = $search = "<!--GETIMG_TAG_$i-->";
		$this->replace_code['replace'][$i] = "<?php echo setTplImageFormat($id,$width,$height,$gen,$is_full); ?>";
		return $search;
	}
