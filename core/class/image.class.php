...
class Image
{
	...
	function save()
	{
		...
		elseif(!$this->saveFile($this->file['tmp_name'], $this->file['local_target'],$this->file['is_convert']))
			$this->error_code = -103;
		elseif($this->file['is_image'] && (!$this->file['image_info'] = $this->getImageInfo($this->file['local_target'])))
		{
	...
	public function convertType($source,$target)
	{
        ...
        if($info !== false)
		{
			...
			if('gif'==$type || 'png'==$type)
			{
				...
				$background_color = imagecolorallocate($tempImg,255,255,255);
				imagefill($tempImg , 0 , 0 , $background_color);
				imagecopyresized($tempImg, $srcImg, 0, 0, 0, 0,$width, $height, $width, $height);
				imagedestroy($srcImg);
				$srcImg = &$tempImg;
			}
	...
	public function thumb($image,$maxWidth=200,$maxHeight=50,$gen = 0,$interlace=true,$filepath = '')
    {
        ...
        if($info !== false)
		{
			...
			elseif($gen == 2)
            {
				...
				{
					$height = $maxHeight;
					$srcHeight = $srcWidth * $height / $width;
				}
            }
