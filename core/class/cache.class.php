...
class Cache
{
	...
	public function saveCache($cache_name, $data)
	{
		...
		if($isfile_cache)
		{
			...
			writeFile($this->cache_path.$cache_name.'.cache.php',(string)$cache_data);
		}
	}
	...
	public function updateCache($cache_names = '')
	{
		...
		if(!$update_list)
		{
			...
			bindCacheSetting();
			@require_once fimport('cache/seo');
			bindCacheSeo();
                        Cache::getInstance()->loadCache('seos',true);
			$cache_dir = FANWE_ROOT.'./core/cache';
			$cache_dir_handle = dir($cache_dir);
			while($entry = $cache_dir_handle->read())
			{
				if(!in_array($entry, array('.', '..')) && preg_match("/^([\w]+)\.cache\.php$/", $entry, $entryr) && $entryr[1] != 'setting' && $entryr[1] != 'seo' && substr($entry, -10) == '.cache.php' && is_file($cache_dir.'/'.$entry))
				{
