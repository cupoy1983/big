 ...
 * @return string
 */
function FU($type,$args = array(),$is_full = false)
{
	global $_FANWE;
	...
	if ($seo_modules === NULL)
	{
		if(!isset($_FANWE['cache']['seos']))
			Cache::getInstance()->loadCache('seos');
	...
	else
	{
		...
		switch($module)
		{
			...
			case 'book':
				$params = array('cate' => '','gid' => '','sid' => '','price'=>'','color'=>'','sort'=>'','tag' => '','page' => 0);
				$search = array('cate/','gid/','sid/','price/','color/','sort/','tag/','page/');
				$replace = array('','g','s','p','c','','','');
				if($action == 'shopping')
					$action = $seo_modules[$module][$action]['action'];
				
				$module = $seo_modules[$module]['module'];
			break;
			...
			case 'look':
			case 'dapei':
				$params = array('type' => '','sort' => '','page' => 0);
				$search = array('/index','/type','/sort','/page');
				$replace = array('','','','');
				$module = $seo_modules[$module]['module'];
			break;
			...
			case 'group':
				switch($action)
				{
					...
					case 'detail':
						$params = array('type' => '','fid' => 0,'sort' => '','page' => 0);
						$search = array('detail/','type/','fid/','sort/','page/');
						$replace = array('','','','','');
					break;
		
					case 'users':
					case 'apply':
						$params = array('fid' => 0,'page' => 0);
						$search = array('fid/','page/');
						$replace = array('','');
					break;
				...
				elseif($action == 'msgview')
                                {
                                    $url = $site_url.$module.$depr.$action;
                                    if($args['lid'] > 0)
                                        $url .= $depr.'lid'.$depr.$args['lid'];
                                    
                                    if($args['mid'] > 0)
                                        $url .= $depr.'mid'.$depr.$args['mid'];
		...
		if($is_url_app)
			$url .= $module.(empty($action) ? '' : $depr.$action).$query;
    ...
    if($type == 'tgo/index')
	{
		...
		if(isset($args['sid']) && (int)$args['sid'] > 0)
			$tk['sid'] = (int)$args['sid']; 

		if(isset($args['gid']) && (int)$args['gid'] > 0)
			$tk['gid'] = (int)$args['gid'];
		...
		if(count($tk) > 0)
			$url .= '&args='.base64_encode(serialize($tk));
	}
 ...
 * @return string
 */
function getTplCache($file, $args = array(), $is_dynamic = 0,$dir = '')
{
	global $_FANWE;
	...
	if(!empty($args))
	{
		$tpl_file .= http_build_query($args);
	}
	$filename = md5($tpl_file);
	return PUBLIC_ROOT.'./data/tpl/caches/'.$dir.str_replace('/', '_', $file).'/'.substr($filename,0,2)."/".substr($filename,2,2)."/".$filename.".htm";
}
...
function display($cache_file = '',$is_session = true,$is_return = false)
{
	...
	$_FANWE['config']['gzip_compress'] ? ob_start('ob_gzhandler') : ob_start();
	echo $content;
}
...
function express(&$content)
{
	...
	if(!$express)
	{
		$express_rs = FDB::fetchAll("select `emotion`,concat('".$_FANWE['site_root']."public/expression/',`type`,'/',`filename`) as fname from ".FDB::table('expression'));
	...
	if(!empty($events[1]))
	{
		...
		foreach($events as $event)
		{
			if(!empty($event))
			{
				if(substr($event,0,3) != '35;' && substr($event,0,3) != '64;')
		...
		{
			$patterns[] = '#'.$data['title'].'#';
			$replace[] = '<a href="'.FU("event/detail",array("id"=>$data['id'])).'" target="_blank">&#35;'.str_replace('@','&#64;',$data['title']).'&#35;</a>';
		}
	...
	if(count($_FANWE['tpl_user_formats']) > 0)
	{
		...
		if(!empty($user_ids))
		{
			$res = FDB::query("SELECT u.uid,u.gid,u.user_name,u.avatar,u.reg_time,u.credits,u.is_daren,u.is_buyer,uc.* FROM ".FDB::table('user').' AS u 
				INNER JOIN '.FDB::table('user_count').' AS uc USING(uid) 
				WHERE u.uid IN ('.$user_ids.')');
...
function setTplFormats($format_key,$format_list)
{
	global $_FANWE;
	foreach($format_list as $rkey => $rformats)
	{
		if(is_array($rformats))
		{
			foreach($rformats as $ckey => $cformats)
			{
				if(!isset($_FANWE[$format_key][$rkey][$ckey]))
					$_FANWE[$format_key][$rkey][$ckey] = $cformats;
			}
		}
	}
}
...
function getUserFormatHtml($user,$format)
{
	...
	if($format['is_mark'] == 1)
	{
		$group = $_FANWE['cache']['user_group'][$user['gid']];
		if(!empty($group['icon']))
		{
			$mark_html .= '<img alt="'.$group['name'].'" src="./public/icons/'.$group['icon'].'" class="v">';
		}
		elseif($user['is_daren'] == 1)
		{
			if($daren_name === NULL)
				$daren_name = sprintf(lang('user','daren_alt'),$_FANWE['setting']['site_name']);
			$mark_html .= '<a href="'.FU('daren/apply').'" class="v" target="_blank"><img title="'.$daren_name.'" src="./tpl/images/daren_icon.png" class="v"></a>';
		}
		elseif($user['is_buyer'] == 1)
			$mark_html .= '<a href="'.FU('settings/buyerverifier').'" class="v" target="_blank"><img title="'.lang('user','buyer_alt').'" src="./tpl/images/buyer_icon.png" class="v"></a>';
	}
	...
	else
	{
		...
		elseif($format['type'] == 2)
		{
			$html = '<a href="javascript:;" onclick="$.AtMe_Share(this);" class="home_at_ta bc" toname="'.$user_name.'" >@'.$user_name.'</a>';
		}
		else
		{
			$width = '';
			...
			if($format['type'] == 3)
			{
				if($is_lazyload === FALSE)
					$html = '<img class="'.$img_class.'" src="'.setTplAvatarFormat($user['avatar'],$format['img_type']).'" '.$width.' />';
				else
					$html = '<img class="'.$img_class.'" original="'.setTplAvatarFormat($user['avatar'],$format['img_type']).'" src="./tpl/images/lazyload.gif" '.$width.'/>';
			}
			elseif($format['type'] == 4)
			{
				$html = '<a '.$link_class.'title="'.$user_name.'" href="'.$user['url'].'" target="_blank">@'.$user_name.'</a>';
			}
			else
			{
				if($is_lazyload === FALSE)
					$html = '<a '.$link_class.'title="'.$user_name.'" href="'.$user['url'].'" target="_blank"><img class="GUID '.$img_class.'" uid="'.$uid.'" src="'.setTplAvatarFormat($user['avatar'],$format['img_type']).'" '.$width.' alt="'.$user_name.'"/></a>';
				else
					$html = '<a '.$link_class.'title="'.$user_name.'" href="'.$user['url'].'" target="_blank"><img class="GUID '.$img_class.'" uid="'.$uid.'" original="'.setTplAvatarFormat($user['avatar'],$format['img_type']).'" src="./tpl/images/lazyload.gif" '.$width.' alt="'.$user_name.'"/></a>';
			}
		}
		$html .= $mark_html;
	}
...
//加入图片格式化
function setTplImageFormat($id,$width,$height,$gen,$is_full)
{
	global $_FANWE;
	$id = (int)$id;
	$width = (int)$width;
	...
	$key = md5($width.'_'.$height.'_'.$gen.'_'.$is_full);
	$_FANWE['tpl_image_formats'][$id][$key] = array(
		'width' => $width,
		'height'=>$height,
		'gen' => $gen,
		'is_full' => $is_full
...
function setTplAvatarFormat($id,$type)
{
	global $_FANWE;
	$id = (int)$id;
	$type = trim($type);
	...
	if($id == 0)
	{
		...
		{
			case 'm':
				return './public/upload/avatar/noavatar_middle.jpg';
			break;
			case 'b':
				return './public/upload/avatar/noavatar_big.jpg';
			break;
			default:
				return './public/upload/avatar/noavatar_small.jpg';
			break;
		}
	}
	...
	if($types === NULL)
	{
		FS("Image")->getImageArgs($types,'avatar');
		$types = $types['types'];
	}
	...
	return setTplImageFormat($id,$size,$size,1,0);
}

function formatImageTags(&$content)
{
	global $_FANWE;
	if(isset($_FANWE['tpl_image_formats']) && count($_FANWE['tpl_image_formats']) > 0)
	{
		$patterns = array();
		$replace = array();
		
		$img_ids = array_keys($_FANWE['tpl_image_formats']);
		$img_list = FS('Image')->getImageListByIds($img_ids);
		if(count($img_list) > 0)
		{
			foreach($img_list as $img_id => $img)
			{
				foreach($_FANWE['tpl_image_formats'][$img_id] as $img_key => $img_val)
				{
					$patterns[] = "<!--IMAGE_".$img_id."_".$img_key."-->";
					$replace[] = formatImage($img,$img_val['width'],$img_val['height'],$img_val['gen'],$img_val['is_full']);
				}
			}
			$content = str_replace($patterns,$replace,$content);
		}
...
function getAdvPosition($id,$count = '',$target='')
{
	...
	if(is_numeric($id))
		...
		$ap = FDB::fetchFirst('SELECT * FROM '.FDB::table('adv_position')." WHERE code ='".$id."'");
	...
	$where = "status = 1 AND position_id = '".$ap['id']."'";
	if($target != '')
 ...
 * @param string $expired_time 有效时长(秒),0 为不过期
 * @return bool
 */
function setCache($key,$data,$expired_time = 0)
{
	$cache_path = PUBLIC_ROOT.'./data/caches/custom/'.$key.'.cache.php';
	...
	$cache_data = array();
	$cache_data['time'] = TIMESTAMP;
	$cache_data['expired_time'] = 0;
	if($expired_time > 0)
		$cache_data['expired_time'] = TIMESTAMP + $expired_time;
	$cache_data['data'] = $data;
	
	return writeFile($cache_path,@serialize($cache_data));
}

/**
 * 生成临时缓存 (2小时后清除)
 */
function setTempCache($key,$data)
{
	$dir = './public/upload/temp/'.fToDate(NULL,'Y/m/d/H');
	if(empty($key))
		$key = md5(microtime(true)).random('6');
	$file_name = $key.'.cache.php';
	makeDir(FANWE_ROOT.$dir);
	$cache_path = FANWE_ROOT.$dir."/".$file_name;
	writeFile($cache_path,@serialize($cache_data));
...
{
	$dir = './public/upload/temp/'.fToDate(NULL,'Y/m/d/H');
	$cache_path = FANWE_ROOT.$dir."/".$key.'.cache.php';
 ...
 * 删除临时缓存
 */
function deleteTempCache($key)
{
	$dir = './public/upload/temp/'.fToDate(NULL,'Y/m/d/H');
	$cache_path = FANWE_ROOT.$dir."/".$key.'.cache.php';
	return removeFile($cache_path);
}
...
function avatar($img_id, $type = 'm',$is_full = false)
{
	$img_id = (int)$img_id;
	if($img_id == 0)
	{
		...
		{
			case 'm':
				$src = './public/upload/avatar/noavatar_middle.jpg';
			break;
			case 'b':
				$src = './public/upload/avatar/noavatar_big.jpg';
			break;
			default:
				$src = './public/upload/avatar/noavatar_small.jpg';
			break;
		}

		if($is_full)
			return FS("Image")->getImageUrl($src,2);
		else
			return $src;
	}
	...
	return getImgById($img_id,$size,$size,1,$is_full);
}
...
{
	$img['src'] = $img_url;
	return formatImage($img,$width,$height,$gen,$is_full);
}

/**
 * 根据图片编号。 获取规格图片的地址，如果没有返回动态生成路径。
 * by fzmatthew
 */
function getImgById($id,$width=0,$height=0,$gen = 0,$is_full = 0)
{
	static $imgs = array();
	if(!isset($imgs[$id]))
	{
		$imgs[$id] = FS("Image")->getImageById($id);
	}
	return formatImage($imgs[$id],$width,$height,$gen,$is_full);
}

function formatImage($img,$width=0,$height=0,$gen = 0,$is_full = 0)
{
	if(!$img)
		return '';
	...
	{
		if($is_full)
			$img_url = FS("Image")->getImageUrl($img['src'],2);
		else
			$img_url = $img['src'];
		return $img_url;
	}

	$img_url_arr[0] = substr($img['src'],0,-4);
	$img_url_arr[1] = substr($img['src'],-3,3);
	$img_url = $img_url_arr[0]."_".$width."x".$height.".".$img_url_arr[1];
	if(strpos($img['src'],'./public/') === FALSE)
	{
		if(strpos($img['src'],'./upyun/') !== FALSE)
		{
			$img_url = $img['src'].'!'.$width."x".$height;
			if($is_full)
				$img_url = str_replace('./upyun/',$_FANWE['UPYUN_SETTING']['url'].'/',$img_url);
		}
		else
		{
			$server = FS("Image")->getServerByUrl($img['src']);
			...
			$img_url = $server['url'].'getimg.php?img='.str_replace('./'.$server['code'].'/','',$img_url)."&gen=".$gen;
		}
	}
	else
	{
		if(!file_exists(FANWE_ROOT.$img_url))
		{
			...
			if($is_full)
				$img_url = $_FANWE['site_url']."getimg.php?img=".$img_url."&gen=".$gen;
			else
				$img_url = $_FANWE['site_root']."getimg.php?img=".$img_url."&gen=".$gen;
		}
		elseif($is_full)
			$img_url = FS("Image")->getImageUrl($img_url,2);
	}
...
function copyFile($origin_path, $path = 'share',$del_temp = true,$id = 0)
{
	...
	{
		$file_name = md5(microtime(true)).random('6').'.jpg';
		$file_path1 = FANWE_ROOT.$dir."/".$file_name;

		if(!$image->convertType($file_path,$file_path1))
			return false;
		else
		{
			@unlink($file_path);
 ...
 * by fzmatthew
 */
function copyImage($origin_path,$sizes = array(),$path = 'photos', $del_temp = true,$id = 0)
{
	global $_FANWE;
...
//获取url的内容
function getUrlContent($url,$is_location = false,$referer = '')
{
	$content = '';
	if(!parseUrl($url))
	{
...
function getImageLength($url)
{
	...
	if(parseUrl($url))
	{
		...
		$head = curl_exec($ch);
		curl_close($ch);
		preg_match("/Content-Length:\s*(\d+)\s/m",$head,$size);
		if($size)
			$size = floatval($size[1]);
	}
	else
	{
		if(strpos($url,FANWE_ROOT) === FALSE)
			$url = FANWE_ROOT.$url;
		$size = floatval(@filesize($url));
	}
...
function formatPageSeo()
{
	global $_FANWE;
	$module = strtolower(MODULE_NAME);
	$action = ACTION_NAME;
	
	$seo_title = (empty($_FANWE['nav_title']) ? '' :  $_FANWE['nav_title']. '-') . $_FANWE['setting']['site_title'];
	$seo_keywords = $_FANWE['seo_keywords'].$_FANWE['setting']['site_keywords'];
	$seo_description = $_FANWE['seo_description'].$_FANWE['setting']['site_description'];
	
	if($module == 'note')
		$action = 'all';
	
	$seos_cache = $_FANWE['cache']['seos'];
	
	if(isset($seos_cache[$module]) && (isset($seos_cache[$module][$action]) || isset($seos_cache[$module]['all'])))
	{
		$seos = array();
		$seos['GLOBAL_NAME'] = $_FANWE['setting']['site_name'];
		$seos['GLOBAL_TITLE'] = $_FANWE['setting']['site_title'];
		$seos['GLOBAL_KEYWORDS'] = $_FANWE['setting']['site_keywords'];
		$seos['GLOBAL_DESCRIPTION'] = $_FANWE['setting']['site_description'];
		
		if(isset($_FANWE['PAGE_SEO_SELF']))
		{
			foreach($_FANWE['PAGE_SEO_SELF'] as $name => $val)
			{
				if(!is_array($val))
					$seos[strtoupper($name)] = $val;
			}
		}
		else
		{
		...
		if(isset($seos_cache[$module]['all']))
			$action = 'all';
		
		
		static $i=0;
		foreach($seos_cache[$module][$action]['seo']['title'] as $item)
		{
			foreach($item['args'] as $v){
				$title_rule['args'][$i]=$v;
				$i++;
			}
			$title_rule['content'] .=$item['content'];
			$bln = true;
			foreach($item['args'] as $arg)
			{
				if(!isset($seos[$arg]) || empty($seos[$arg]))
				{
					$bln = false;
					break;
				}
		...
		$seo_title = $title_rule['content'];
		$search = array();
		$replace = array();
		foreach($title_rule['args'] as $arg)
		{
			$search[] = '{'.$arg.'}';
			$replace[] = $seos[$arg];
		}
		...
		static $j=0;
		foreach($seos_cache[$module][$action]['seo']['keywords'] as $item)
		{
			
			foreach($item['args'] as $v){
				$keywords_rule['args'][$j]=$v;
				$j++;
			}
			$keywords_rule['content'] .=$item['content'];
			$bln = true;
			foreach($item['args'] as $arg)
			{
				if(!isset($seos[$arg]) || empty($seos[$arg]))
		...
		$seo_keywords = $keywords_rule['content'];
		$search = array();
		$replace = array();
		foreach($keywords_rule['args'] as $arg)
		{
			$search[] = '{'.$arg.'}';
			$replace[] = $seos[$arg];
		}
		$seo_keywords = str_replace($search,$replace,$seo_keywords);
		static $k=0;
		foreach($seos_cache[$module][$action]['seo']['description'] as $item)
		{
			foreach($item['args'] as $v){
				$description_rule['args'][$k]=$v;
				$k++;
			}
			$description_rule['content'] .=$item['content'];
			$bln = true;
			foreach($item['args'] as $arg)
			{
				if(!isset($seos[$arg]) || empty($seos[$arg]))
		...
		$seo_description = $description_rule['content'];
		$search = array();
		$replace = array();
		foreach($description_rule['args'] as $arg)
		{
			$search[] = '{'.$arg.'}';
			$replace[] = $seos[$arg];
		}
		$seo_description = str_replace($search,$replace,$seo_description);
	}
...
{
	$list = array();
	$res = FDB::query('SELECT word,brief,url,target FROM '.FDB::table('anchor').' WHERE status = 1 ORDER BY word ASC');
	while($data = FDB::fetch($res))
	{
		$temp = &$list;
		...
		if(!isset($temp[',']))
			$temp[','] = array('desc'=>$data['brief'],'url'=>$data['url'],'target'=>$data['target']);
	}
	
	foreach($list as $item => $val)
	{
		@file_put_contents(FANWE_ROOT.'public/table/anchor/'.urlencode($item).'.table','<?php '."\nreturn ".(string)var_export($val,true).";\n".'?>');
	}
}

function formatAnchorLink($content,$length)
{
	if(empty($content))
		return '';
	...
	$patterns = array();
	$replace = array();
	preg_match_all("/<(.+?)\s.+?>(.+?)<\/\\1>/",$content,$htmls);
	if(!empty($htmls[0]))
	{
		$htmls = array_unique($htmls[0]);
		foreach($htmls as $html)
		{
			$patterns[] = md5($html,true);
			$replace[] = $html;
		}
	}
	
	$content = str_replace('　',' ',$content);
	$content = str_replace($replace,$patterns,$content);
	...
	while($bln)
	{
		...
		if(file_exists(FANWE_ROOT.'public/table/anchor/'.urlencode($char1).'.table'))
		{
			$charindex = $index + 1;
			$words = include FANWE_ROOT.'public/table/anchor/'.urlencode($char1).'.table';
			$wbln = true;
			$temp = &$words;
			$word = $char;
			$anchor = NULL;
			...
			while($wbln)
			{
				$char = mb_substr($content,$charindex,1,'UTF-8');
				$char1 = strtolower($char);
				if(isset($temp[$char1]))
				{
					$word .= $char;
			...
			if($anchor !== NULL)
				$word = '<a href="'.$anchor['url'].'" title="'.$anchor['desc'].'" target="'.$anchor['target'].'">'.$word.'</a>';
	...
	return str_replace($patterns,$replace,$newcontent);
}

function getTdjGoods($goods)
{
	global $_FANWE;
	$tdj = "";
	if($goods['type'] == "taobao")
	{
		$tdj = ' biz-itemid="'.str_replace("taobao_","",$goods['keyid']).'" data-rd="'.$_FANWE['cache']['business']['taobao']['rd'].'"';
	}
	return $tdj;
}
?>