 ...
 * @return  string
 */
function getBeforeTimelag($time,$is_seconds = false)
{
	if($time == 0)
		return "";
	...
	if($today_time === NULL)
	{
		...
		$seconds_lang = lang('time','seconds');
		$months_lang = lang('time','months');
		$date_lang = lang('time','date');
	}
	
	$app_seconds = $is_seconds ? ':s' : '';
	...
	if($year_time < $today_year)
		return fToDate($time,'Y-m-d H:i'.$app_seconds);
		
	$timelag_str = fToDate($time,' H:i'.$app_seconds);
	...
	switch($day_time)
	{
		...
		default:
			$timelag_str = fToDate($time,'m'.$months_lang.'d'.$date_lang.' H:i'.$app_seconds);
		break;
	}
	return $timelag_str;
}
