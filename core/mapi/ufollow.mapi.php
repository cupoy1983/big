...
class ufollowMapi
{
	public function run()
	{
		...
		foreach($share_list as $k=>$v){
			...
			if(!$share_list[$k]['is_relay']){
				$share_list[$k]['is_relay']=array();
			}
			if(!$share_list[$k]['is_parent']){
				$share_list[$k]['is_parent']=array();
			}
			if(!$share_list[$k]['relay_share']){
				$share_list[$k]['relay_share']=array();
			}
		}
		$root['return'] = 1;
