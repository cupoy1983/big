...
class initMapi
{
    ...
    public function run()
	{
        ...
        */
        ...
        $root['index_logo'] = FS("Image")->getImageUrl($_FANWE['MConfig']['index_logo'],2);
        $root['newslist'] = $_FANWE['MConfig']['newslist'];//新闻列表
        //$root['addr_tlist'] = $_FANWE['MConfig']['addr_tlist'];//保存地址标题
        
        //$root['adv_youhui'] = m_adv_youhui();
		
		$root['tqq_app_key'] = $_FANWE['MConfig']['tqq_app_key'];
		$root['tqq_app_secret'] = $_FANWE['MConfig']['tqq_app_secret'];
		$root['tqq_redirect_uri'] = $_FANWE['MConfig']['tqq_redirect_uri'];
		$root['tqq_status'] = 1;
		if(empty($root['tqq_app_key']) || empty($root['tqq_app_key']) || empty($root['tqq_app_key']))
			$root['tqq_status'] = 0;
		
		$root['sina_app_key'] = $_FANWE['MConfig']['sina_app_key'];
		$root['sina_app_secret'] = $_FANWE['MConfig']['sina_app_secret'];
		$root['sina_redirect_uri'] = $_FANWE['MConfig']['sina_redirect_uri'];
		$root['sina_status'] = 1;
		if(empty($root['sina_app_key']) || empty($root['sina_app_key']) || empty($root['sina_app_key']))
			$root['sina_status'] = 0;
