...
function mGetShareDetailList($list,$is_parent = false,$img_width = 160,$img_height=160)
{
	...
	foreach($list as $item)
	{
		...
		if(!empty($item['cache_data']['imgs']))
		{
			...
			{
				unset($img['url']);
				$shares[$share_id]['imgs'][$img_index] = $img;
				
				if($img['type'] == 'g')
				{
					$format_goods[$img['goods_id']][] = &$shares[$share_id]['imgs'][$img_index];
				}
				else
				{
					$shares[$share_id]['imgs'][$img_index]['name'] = '';
					$shares[$share_id]['imgs'][$img_index]['price'] = '';
					$shares[$share_id]['imgs'][$img_index]['goods_url'] = '';
					$shares[$share_id]['imgs'][$img_index]['taoke_url'] = '';
					$shares[$share_id]['imgs'][$img_index]['price_format'] = '';
					$shares[$share_id]['imgs'][$img_index]['commission'] = '';
					$shares[$share_id]['imgs'][$img_index]['commission_format'] = '';
				}
				$format_images[$img['img_id']][] = &$shares[$share_id]['imgs'][$img_index];
				$img_index++;
			}
		}
	...
	if(count($rec_ids) > 0)
	{
		...
		foreach($intersects as $share_id)
		{
			...
			foreach($rec_shares_ids[$share_id]['imgs'] as $img_key => $img)
			{
				if($img['type'] == 'g')
					$format_goods[$img['goods_id']][] = &$rec_shares_ids[$share_id]['imgs'][$img_key];
				$format_images[$img['img_id']][] = &$rec_shares_ids[$share_id]['imgs'][$img_key];
			}
			$temp_ids[] = $share_id;
		}
		...
		if(count($diffs) > 0)
		{
			...
			while($item = FDB::fetch($res))
			{
				...
				if(!empty($item['cache_data']['imgs']))
				{
					...
					{
						unset($img['url']);
						$rec_shares_ids[$share_id]['imgs'][$img_index] = $img;
						if($img['type'] == 'g')
						{
							$format_goods[$img['goods_id']][] = &$rec_shares_ids[$share_id]['imgs'][$img_index];
						}
						else
						{
							$rec_shares_ids[$share_id]['imgs'][$img_index]['name'] = '';
							$rec_shares_ids[$share_id]['imgs'][$img_index]['price'] = '';
							$rec_shares_ids[$share_id]['imgs'][$img_index]['goods_url'] = '';
							$rec_shares_ids[$share_id]['imgs'][$img_index]['taoke_url'] = '';
							$rec_shares_ids[$share_id]['imgs'][$img_index]['price_format'] = '';
							$rec_shares_ids[$share_id]['imgs'][$img_index]['commission'] = '';
							$rec_shares_ids[$share_id]['imgs'][$img_index]['commission_format'] = '';
						}
						$format_images[$img['img_id']][] = &$rec_shares_ids[$share_id]['imgs'][$img_index];
						$img_index++;
					}
				}
		...
		foreach($image_list as $img_id => $img)
		{
			foreach($format_images[$img_id] as $key => $val)
			{
				...
				if($img['width'] > $img_width)
					$format_images[$img_id][$key]['width'] = 160;
				else
					$format_images[$img_id][$key]['width'] = $img['width'];
				
				$format_images[$img_id][$key]['small_img'] = getImgName($img['src'],$img_width,$img_height,0,true);
