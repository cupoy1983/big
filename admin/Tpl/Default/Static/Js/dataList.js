jQuery(function($){
	$(document).mousemove(function(e){
		var obj = e.target;
		if ($(obj).attr('tagName') && $(obj).attr('tagName').toLowerCase() == 'span' && 
...
function toggleStatus(obj,id,name,args)
{
	if($('img',obj).length == 0)
		return false;
	...
	$.ajax({
		...
		success: function(result){
			if(result.isErr == 0)
			{
				...
				img.src = src;
				if(FANWE.IS_TOGGLE_FUNC)
				{
					query.val = result.content;
					FANWE.IS_TOGGLE_FUNC.call(null,query);
				}
				else if(FANWE.IS_TOGGLE_STATUS_RELOAD)
				{
					var fun = function(){
...
function toggleStatusOther(obj,id,name,val)
{
	var module = obj.getAttribute('module');
	var query = new Object();
	query.field = name;
