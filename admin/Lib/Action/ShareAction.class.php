...
class ShareAction extends CommonAction
{
	public function image()
	{
		...
		if(!$is_empty)
		{
			...
			$sql = 'SELECT share_id,uid,content,create_time,collect_count,relay_count,
				comment_count,type,share_data,status,is_index,cache_data 
			...
			$sql = 'SELECT sc.share_id,gc.name FROM '.C("DB_PREFIX").'share_color AS sc 
				INNER JOIN '.C("DB_PREFIX").'goods_color AS gc ON gc.id = sc.color_id 
				WHERE sc.share_id IN ('.implode(',',array_keys($share_ids)).')';
			$cate_list = M()->query($sql);
			foreach($cate_list as $item)
			{
				if(empty($share_ids[$item['share_id']]['color_name']))
					$share_ids[$item['share_id']]['color_name'] = $item['name'];
				else
					$share_ids[$item['share_id']]['color_name'] .= '<br/>'.$item['name'];
			}
		}
	...
	public function dapei()
	{
		...
		if(!$is_empty)
		{
			...
			$sql = 'SELECT share_id,uid,content,create_time,collect_count,relay_count,comment_count,type,share_data,status,is_index,sort,cache_data 
				FROM '.C("DB_PREFIX").'share WHERE share_id IN ('.implode(',',array_keys($share_ids)).')';
			$share_list = M()->query($sql);
			foreach($share_list as $item)
			{
	...
	public function check()
	{
		...
		if(!$is_empty)
		{
			...
			$sql = 'SELECT share_id,uid,content,create_time,collect_count,relay_count,comment_count,type,share_data,cache_data,status 
				FROM '.C("DB_PREFIX").'share WHERE share_id IN ('.implode(',',array_keys($share_ids)).')';
			$share_list = M()->query($sql);
			foreach($share_list as $item)
			{
	...
	function edit()
	{
		...
		$share['cache_data'] = stripslashesDeep(unserialize($share['cache_data']));
		$share['share_tags'] = '';
		...
		FS('Share')->shareImageFormat($share);
		...
		$color_list = array();
		$res = FDB::query("select * from ".FDB::table("goods_color"));
		while($data = FDB::fetch($res))
		{
			$color_list[$data['id']] = $data;
		}
