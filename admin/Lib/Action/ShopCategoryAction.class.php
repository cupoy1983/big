...
class ShopCategoryAction extends CommonAction
{
	...
	{
		$cate_tree = D("ShopCategory")->order('sort asc,id asc')->select();
	...
	public function add()
	{	
		$cate_tree = D("ShopCategory")->where('parent_id = 0')->order('sort asc,id asc')->select();
	...
	{	
		...
		$cate_tree = D("ShopCategory")->where($condition)->order('sort asc,id asc')->select();
		$this->assign("cate_tree",$cate_tree);
...
function getRelatedLink($id,$cate)
{
	return '<a href="'.U('ShopCatesRelated/index',array('cid'=>$id,'cname'=>$cate['name'])).'">'.L('CATE_RELATED').'</a>&nbsp;';
}
?>