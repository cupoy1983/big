...
class MedalAction extends CommonAction
{
	public function add()
	{	
		$user_groups = D("UserGroup")->where('status = 1 AND gid <> 6')->select();
	...
	public function user()
	{
		...
		$medal_list = D('Medal')->where('status = 1')->order('sort asc')->select();
	...
	{
		$medal_list = D('Medal')->where('status = 1 AND give_type = 1')->order('sort asc')->select();
		$this->assign("medal_list",$medal_list);
		$this->display ();
	}
	...
	public function removeAward()
	{
		...
		if(!empty($id))
		{
			...
			$award_list = D('UserMedal')->where($condition )->select();
			$uids = array();
	...
	public function checkApply()
	{
		...
		{
			$ids = explode(',',$id);
			$award_list = D('MedalApply')->where($condition )->select();
			$uids = array();
