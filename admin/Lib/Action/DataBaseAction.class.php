...
class DataBaseAction extends CommonAction
{
	...
	public function dumptable()
	{
		...
		if($begin == 0)
		{
		     ...
		     $tmp_sql  = str_replace(array('CREATE TABLE',C('DB_PREFIX')),array('CREATE TABLE IF NOT EXISTS','%DB_PREFIX%'),$tmp_sql);
			 $dumpsql_vol .= $tmp_sql;   //表结构语句处理结束
			 $dumpsql_vol .= "TRUNCATE TABLE `$tbname`;\r\n";  //用于表结构导出处理的Sql语句
		...
		if($data_num > $begin)
		{
			...
			$data_list=$model->limit($begin.','.$limit)->select();
	...
	public function restoretable()
	{
		...
		if($begin == 0)
		{
			 ...
			 if(strpos($sql,'ENGINE=InnoDB') !== FALSE)
				$_SESSION['restore_table_commit'] = 1;
			 else
				$_SESSION['restore_table_commit'] = 0;
			 ...
			 foreach($segmentSql as $itemSql)
			 {
				 ...
				 if(strpos($itemSql,'DROP TABLE IF EXISTS') !== FALSE)
				 {
					  $itemSql = str_replace('DROP TABLE IF EXISTS','TRUNCATE TABLE',$itemSql);
				 }
				 elseif(strpos($itemSql,'CREATE TABLE') !== FALSE && strpos($itemSql,'CREATE TABLE IF NOT EXISTS') === FALSE)
				 {
					 $itemSql = str_replace('CREATE TABLE','CREATE TABLE IF NOT EXISTS',$itemSql);
				 }

				 $itemSql = str_replace('USING BTREE','',$itemSql);
		...
		if(file_exists($table_dir.$begin.'.sql'))
		{
			...
			$segmentSql = explode(";\n", $sql);
			$sql_index = 0;
			if($_SESSION['restore_table_commit'] == 1)
				$db->query("BEGIN");
