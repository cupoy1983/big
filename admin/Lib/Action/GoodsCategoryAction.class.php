...
class GoodsCategoryAction extends CommonAction
{
	...
	public function edit()
	{
		...
		$childs[] = $id;
		$cate_list = D("GoodsCategory")->where('status = 1 AND cate_id not in ('.implode(',', $childs).')')->field('cate_id,parent_id,cate_name')->order('sort ASC,cate_id ASC')->select();
	...
	public function remove()
	{
		...
		if(!empty($id))
		{
			...
			else
			{
				...
				{
					D("GoodsCatesGl")->where($condition)->delete();
					$this->saveLog(1,$id);
				}
				else
				{
	...
	public function updateCache()
	{
		...
		$this->assign ('jumpUrl',U('GoodsCategory/index'));
...
function getHandlerLinks($id,$cate)
{
	$links = '';
	if($cate['is_root'] == 0)
		$links .= ' <a href="'.U('IndexCateShare/index',array('cate_id'=>$id)).'">'.L('INDEX_CATE_SHARE').'</a>';
	
	$links .= ' <a href="'.U('GoodsCatesGl/index',array('cate_id'=>$id,'cate_name'=>$cate['cate_name'])).'">'.L('CATE_RELATED').'</a>';
	$links .= ' <a href="'.U('GoodsCategoryTags/index',array('cate_id'=>$id)).'">'.L('SETTING_CATE_TAG').'</a>';
	
	if($cate['child'] > 0 || $cate['is_root'] == 1)
		$links .= ' <a href="'.U('Adv/add',array('pcode'=>'book_cate','target_key'=>'cate'.$id,'pcateid'=>$cate['parent_id'])).'">'.L('CATE_ADV').'</a>';
	
	if($cate['child'] > 0)
		$links .= ' <a href="'.U('GoodsCategory/index',array('parent_id'=>$id)).'">'.L('SHOW_CHILD').'</a>';
...
function getAddglLink($id,$cate_name){
    return '<a href="'.U('GoodsCatesGl/index',array('cate_id'=>$id,'cate_name'=>$cate_name)).'">设置关联</a>';
}
?>