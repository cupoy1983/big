...
class UserAction extends CommonAction
{
	public function index()
	{
		...
		if(!empty($user_name))
		{
			...
			$match_key = FS('Words')->segmentToUnicode($user_name,'+');
			$where.=" match(um.user_name) against('".$match_key."' IN BOOLEAN MODE) ";
			$is_match = true;
		}
		...
		if($is_match)
			$sql = 'SELECT COUNT(u.uid) AS tcount FROM '.C("DB_PREFIX").'user_match AS um 
				INNER JOIN '.C("DB_PREFIX").'user AS u ON u.uid = um.uid '.$where;
		else
			$sql = 'SELECT COUNT(u.uid) AS tcount FROM '.C("DB_PREFIX").'user AS u '.$where;

		$count = $model->query($sql);
		$count = $count[0]['tcount'];

		if($is_match)
			$sql = 'SELECT u.* FROM '.C("DB_PREFIX").'user_match AS um 
				INNER JOIN '.C("DB_PREFIX").'user AS u ON u.uid = um.uid '.$where;
		else
			$sql = 'SELECT u.* FROM '.C("DB_PREFIX").'user AS u '.$where;

		$this->_sqlList($model,$sql,$count,$parameter,'u.uid');
	...
	public function insert()
	{
		...
		if ($uid!==false)
		{
			...
			$user_name_match = FS('Words')->segmentToUnicode($data['user_name']);
	...
	public function edit()
	{
		...
		$authoritys = L('AUTHORITYS');
		$ua_list = array();
		$u_authoritys = D('UserAuthority')->where('uid = '.$uid)->select();
	...
	public function update()
	{
		...
		if (false !== $list)
		{
			...
			{
				D('User')->deleteUserAvatar($old_user);
				D("User")->where('uid = '.$uid)->setField('avatar',0);
			}
				
			if(!empty($avatar_img))
			{
				if($old_user['avatar'] == 0)
					FS('User')->saveAvatar($uid,FANWE_ROOT.$avatar_img);
				else
				{
					$img = array();
					$img['id'] = $old_user['avatar'];
					$img['type'] = 'avatar';
					$img['src'] = FANWE_ROOT.$avatar_img;
			...
			D('UserMatch')->where('uid = '.$uid)->save($user_match);
	...
	public function remove()
	{
		...
		{
			case "album":
				$list = D('Album')->where($condition)->limit('0,'.$count)->select();
				if(count($list) > 0)
				{
				...
				echoFlush('<script type="text/javascript">showmessage(\''.U('User/remove',array('id'=>$id,'type'=>'group','index'=>0)).'\',2);</script>');
			break;
			
			case "group":
				$list = D('Forum')->where($condition)->limit('0,'.$count)->select();
				if(count($list) > 0)
				{
					echoFlush('<script type="text/javascript">showmessage(\''.sprintf(L('DELETE_TIPS_5'),$min,$max).'\',1);</script>');
					Vendor("common");
					@set_time_limit(0);
					if(function_exists('ini_set'))
						ini_set('max_execution_time',0);
					...
					foreach($list as $item)
					{
						FS("Group")->removeGroup($item['fid']);
						usleep(10);
					}
					usleep(100);
					$index++;
					echoFlush('<script type="text/javascript">showmessage(\''.U('User/remove',array('id'=>$id,'type'=>'group','index'=>$index)).'\',2);</script>');
				}
				echoFlush('<script type="text/javascript">showmessage(\''.U('User/remove',array('id'=>$id,'type'=>'share','index'=>0)).'\',2);</script>');
			break;

			case "share":
				$list = D('Share')->where($condition)->limit('0,'.$count)->select();
				if(count($list) > 0)
				{
	...
	public function getUserList()
	{
		...
		if(empty($where))
			$sql = 'SELECT uid,user_name FROM '.C("DB_PREFIX").'user WHERE status = 1 ORDER BY uid DESC limit 0,30';
		else
			$sql = 'SELECT u.uid,u.user_name FROM '.C("DB_PREFIX").'user_match as um 
				INNER JOIN '.C("DB_PREFIX").'user as u ON u.uid = um.uid AND u.status = 1 
				WHERE '.$where.' ORDER BY um.uid DESC limit 0,30';
