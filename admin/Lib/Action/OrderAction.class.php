...
class OrderAction extends CommonAction
{
	public function index()
	{
		...
		if(!empty($name))
		{
            ...
            $where .= ' AND o.data_name LIKE \'%'.$like_name.'%\'';
		}
		...
		if(!empty($user_name))
		{
            ...
            $where .= ' AND o.user_name = \''.$user_name.'\'';
		}
		...
		$sql = 'SELECT COUNT(o.id) AS tcount
			FROM '.C("DB_PREFIX").'order as o '.$where;

		$count = $model->query($sql);
		$count = $count[0]['tcount'];

		$sql = 'SELECT o.*,u.avatar FROM '.C("DB_PREFIX").'order as o 
			LEFT JOIN '.C("DB_PREFIX").'user as u ON u.uid = o.uid '.$where;
...
{
	return L("ORDER_TYPE_".$type);
}

function getOrderEdit($id,$status)
{
	if($status == 0)
		return "<a href='javascript:showOrder(".$id.");'>". L('EDIT')."</a>&nbsp;&nbsp;<a href=\"javascript:;\" onclick=\"removeData(this,'$id','id')\">". L('REMOVE')."</a>";
	elseif ($status == 1)
		return "<a href='javascript:showOrder(".$id.");'>". L('VIEW')."</a>";
	else
		return "<a href='javascript:showOrder(".$id.");'>". L('VIEW')."</a>&nbsp;&nbsp;<a href=\"javascript:;\" onclick=\"removeData(this,'$id','id')\">". L('REMOVE')."</a>";
}
?>