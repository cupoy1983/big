...
class GoodsOrderAction extends CommonAction
{
	public function index()
	{
		...
		if(!$is_empty)
		{
			...
			if($count > 0)
			{
				$sql = 'SELECT DISTINCT(order_id),status,settlement_time,goods_id,create_time,goods_name  
					FROM '.C("DB_PREFIX").'goods_order '.$where;
				...
				{
					$users[$temp['uid']] = '';
					$score = (float)$temp['score'] > 0 ? (float)$temp['score'] : L('GET_COMMISSION');
					$pay_time = L('NO_PAY');
					if($temp['pay_time'] > 0)
					...
					if($temp['type'] == 0)
					{
						...
						$orders[$temp['order_id']]['user']['score'] = $score;
						$orders[$temp['order_id']]['user']['is_commission'] = (float)$temp['score'] > 0 ? true : false;
					...
					else
					{
						...
						$orders[$temp['order_id']]['cuser']['score'] = $score;
						$orders[$temp['order_id']]['cuser']['is_commission'] = (float)$temp['score'] > 0 ? true : false;
	...
	public function toggleStatus()
	{
		...
		if(false !== D('GoodsOrder')->where('order_id = '.$id.' AND uid = '.$uid)->setField($field,$val))
		{
			...
			if($field == 'is_pay')
			{
				...
				vendor('common');
				FS('User')->updateUserScore($uid,'GoodsOrder',$action,$msg,$id,$score);
			}
		}
		else
		{
