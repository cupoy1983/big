...
class UserModel extends CommonModel
{
	...
	public function deleteUser($id_arr)
	{
		...
		if(false !== $this->where($condition)->delete())
		{
			...
			D('CommentMe')->where($condition)->delete();
			D('FavMe')->where($condition)->delete();
			D('ForumPost')->where($condition)->delete();
			D('ForumThread')->where($condition)->delete();
			D('ForumThreadBest')->where($condition)->delete();
			D('GoodsOrder')->where($condition)->delete();
			D('IndexCateShare')->where($condition)->delete();
			...
			D('UserAuctionLog')->where($condition)->delete();
			D('UserMoneyLog')->where($condition)->delete();
			
			$list = M()->query('SELECT id FROM '.C("DB_PREFIX").'activity 
				WHERE uid IN ('.$ids.')');
			foreach($list as $data)
			{
				D('ActivityApply')->where('aid = '.$data['id'])->delete();
			}
			D('Activity')->where($condition)->delete();
			D('ActivityPost')->where($condition)->delete();
			D('ActivityApply')->where($condition)->delete();
			
			$list = M()->query('SELECT id FROM '.C("DB_PREFIX").'vote 
				WHERE uid IN ('.$ids.')');
			foreach($list as $data)
			{
				D('VoteOption')->where('vid = '.$data['id'])->delete();
				D('VoteUser')->where('vid = '.$data['id'])->delete();
			}
			D('Vote')->where($condition)->delete();
			D('VotePost')->where($condition)->delete();
			...
			D('UserMatch')->where($condition)->delete();
	...
	public function deleteUserAvatar($user)
	{
		vendor("common");
		FS('Image')->deleteImages(array($user['avatar']));
		$types = array(
			's'=>'small',
			'm'=>'middle',
			'b'=>'big',
		);
		$avatar_path = $this->getUserAvatarPath($user['uid']);
