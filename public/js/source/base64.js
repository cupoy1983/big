(function(global) {
    ...
    var stringToArray = function(s) {
        var a = [];
        for (var i = 0,
        l = s.length; i < l; i++) a[i] = s.charCodeAt(i);
        return a;
    };
    ...
    var convertUTF8ArrayToBase64 = function(bin) {
        ...
        var b64 = [];
        for (var i = 0,
        l = bin.length; i < l; i += 3) {
            var c0 = bin[i],
            c1 = bin[i + 1],
            c2 = bin[i + 2];
            if (c0 >= 256 || c1 >= 256 || c2 >= 256) throw 'unsupported character found';
            var n = (c0 << 16) | (c1 << 8) | c2;
            b64.push(b64charcodes[n >>> 18], b64charcodes[(n >>> 12) & 63], b64charcodes[(n >>> 6) & 63], b64charcodes[n & 63]);
        }
    ...
    var convertBase64ToUTF8Array = function(b64) {
        ...
        for (var i = 0,
        l = b64.length; i < l; i += 4) {
            var n = ((b64tab[b64.charAt(i)] || 0) << 18) | ((b64tab[b64.charAt(i + 1)] || 0) << 12) | ((b64tab[b64.charAt(i + 2)] || 0) << 6) | ((b64tab[b64.charAt(i + 3)] || 0));
            bin.push((n >> 16), ((n >> 8) & 0xff), (n & 0xff));
        }
    ...
    var convertUTF16ArrayToUTF8Array = function(uni) {
        ...
        l = uni.length; i < l; i++) {
            var n = uni[i];
            if (n < 0x80) bin.push(n);
            else if (n < 0x800) bin.push(0xc0 | (n >>> 6), 0x80 | (n & 0x3f));
            else bin.push(0xe0 | ((n >>> 12) & 0x0f), 0x80 | ((n >>> 6) & 0x3f), 0x80 | (n & 0x3f));
        }
        return bin;
    };
        ...
        l = bin.length; i < l; i++) {
            ...
            if (c0 < 0x80) {
                ...
                if (c0 < 0xe0) {
                    ...
                    var c2 = bin[++i];
                    uni.push(((c0 & 0x0f) << 12) | ((c1 & 0x3f) << 6) | (c2 & 0x3f));
                }
            }
        }
        return uni;
