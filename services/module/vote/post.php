...
if(FS('Vote')->getUserIsVote($id,$_FANWE['uid']))
	exit;
unset($_FANWE['request']['share_id']);
$_FANWE['request']['uid'] = $_FANWE['uid'];
$_FANWE['request']['rec_id'] = $id;
$_FANWE['request']['parent_id'] = $vote['share_id'];
$_FANWE['request']['type'] = 'vote_post';
$_FANWE['request']['title'] = addslashes($vote['title']);
$result = array();
...
if($share['status'])
{
	...
	$post_id = (int)FS('Vote')->savePost($id,$content,$share['share_id']);
